...
router.get('/', (req, res) => {
  Post.find()
    ...
    .catch(err =>
      res.status(404).json({ nopostsfound: 'User have not post anyting yet' })
    );
});
...
router.get('/:id', (req, res) => {
  Post.findById(req.params.id)
    ...
    .catch(err =>
      res.status(404).json({ nopostfound: 'No post available for this user' })
    );
});
...
router.post(
  ...
  (req, res) => {
    ...
    // Check Validation (using post validation for comment)
    if (!isValid) {
      // If any errors, send 400 with errors object
      return res.status(400).json(errors);
    }
